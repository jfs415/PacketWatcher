...
public class IpComparator implements RangedComparator<String, String, String> {
    ...
    private boolean validate(String value, String floor, String ceiling, TokenizerEvaluator evaluator) {
        ...
        if (valueTokenizer.countTokens() != floorTokenizer.countTokens()
                || valueTokenizer.countTokens() != ceilingTokenizer.countTokens()) {
            return false;
        }
    ...
    private enum TokenizerEvaluator {
        RANGE_INCLUSIVE {
            @Override
            public boolean processTokenizers(
                    StringTokenizer valueTokenizer, StringTokenizer floorTokenizer, StringTokenizer ceilingTokenizer) {
            ...
            public boolean processTokenizers(
                ...
                while (valueTokenizer.hasMoreTokens()) {
                    ...
                    if ((!valueTokenizer.hasMoreTokens() && (valueOctet <= floorOctet || valueOctet >= ceilingOctet))
                            || tokensNotAlike(
                                    valueTokenizer, floorTokenizer, ceilingTokenizer)) { // Only last octet is bounded
                        return false;
                    }
                }
        ...
        protected boolean tokensNotAlike(
                StringTokenizer valueTokenizer, StringTokenizer floorTokenizer, StringTokenizer ceilingTokenizer) {
            return valueTokenizer.countTokens() != floorTokenizer.countTokens()
                    || valueTokenizer.countTokens() != ceilingTokenizer.countTokens();
        }
    }
}
